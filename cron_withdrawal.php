...
function ChangetoMili($amount,&$currency) {
	switch ($currency)
	{		
	    case "PPCN" : $amount = $amount * 1000;$currency="mPPCN";return $amount;
	    case "mPPCN" :return $amount;
	    case "Satoshi" : $amount = $amount / 100000;$currency="mPPCN";return $amount;
...
//Change to mili pupacoin because asmoney get currencies based on milicoin
...
//			$r = $api->TransferPPCN($address,$amount,'mPPCN','Withdrawal');			
		}
...
{	
	$r = $api->TransferToManyPPCN($btcaddresses,$btcamounts,'mPPCN','Withdrawal');
	if ($r['result'] == APIerror::OK){
			$batchno = $r['value'];
